...
def verify_token(token: str = Depends(security)):
    ...
    return {"user_id": "admin", "role": "admin", "name": "المدير"}

@router.get("/lines")
async def get_all_lines(current_user: dict = Depends(verify_token)):
    """الحصول على جميع الخطوط"""
    try:
        lines = list(lines_collection.find({}, {"_id": 0}).sort("name", 1))
        
        # إضافة إحصائيات لكل خط
        for line in lines:
            line_id = line["id"]
            # عدد المناطق في الخط
            areas_count = areas_collection.count_documents({"line_id": line_id})
            # عدد العيادات في الخط
            clinics_count = clinics_collection.count_documents({"line_id": line_id})
            # عدد المندوبين في الخط
            reps_count = users_collection.count_documents({"line_id": line_id, "role": "medical_rep"})
            ...
            if line.get("manager_id"):
                manager = users_collection.find_one({"id": line["manager_id"]})
...
@router.get("/areas")
async def get_all_areas(current_user: dict = Depends(verify_token)):
    """الحصول على جميع المناطق"""
    try:
        areas = list(areas_collection.find({}, {"_id": 0}).sort("name", 1))
        
        # إضافة معلومات الخط لكل منطقة
        for area in areas:
            line_id = area.get("line_id")
            if line_id:
                line = lines_collection.find_one({"id": line_id})
                area["line_name"] = line["name"] if line else "خط غير محدد"
            else:
                area["line_name"] = "خط غير محدد"
            
            # عدد العيادات في المنطقة
            clinics_count = clinics_collection.count_documents({"area_id": area["id"]})
            area["clinics_count"] = clinics_count
            
            # معلومات المدير
            if area.get("manager_id"):
                manager = users_collection.find_one({"id": area["manager_id"]})
...
async def get_areas_by_line(line_id: str, current_user: dict = Depends(verify_token)):
    ...
    try:
        ...
        areas = list(areas_collection.find({"line_id": line_id}, {"_id": 0}).sort("name", 1))
...
@router.post("/lines")
async def create_line(line_data: LineModel, current_user: dict = Depends(verify_token)):
    """إنشاء خط جديد"""
    try:
        # التحقق من عدم تكرار الكود
        existing_line = lines_collection.find_one({"code": line_data.code})
        if existing_line:
        ...
        result = lines_collection.insert_one(line)
...
@router.post("/areas")
async def create_area(area_data: AreaModel, current_user: dict = Depends(verify_token)):
    """إنشاء منطقة جديدة"""
    try:
        # التحقق من وجود الخط
        line = lines_collection.find_one({"id": area_data.line_id})
        if not line:
            raise HTTPException(status_code=404, detail="الخط غير موجود")
        
        # التحقق من عدم تكرار الكود
        existing_area = areas_collection.find_one({"code": area_data.code})
        if existing_area:
        ...
        result = areas_collection.insert_one(area)
...
async def update_line(line_id: str, line_data: LineUpdateModel, current_user: dict = Depends(verify_token)):
    """تحديث خط موجود"""
    try:
        # التحقق من وجود الخط
        existing_line = lines_collection.find_one({"id": line_id})
        ...
        if line_data.code is not None:
            # التحقق من عدم تكرار الكود
            if line_data.code != existing_line.get("code"):
                existing_code = lines_collection.find_one({"code": line_data.code.upper()})
                if existing_code:
                    raise HTTPException(status_code=400, detail="كود الخط موجود بالفعل")
        ...
        # تحديث الخط
        result = lines_collection.update_one(
            {"id": line_id},
            {"$set": update_data}
        )
        ...
        updated_line = lines_collection.find_one({"id": line_id}, {"_id": 0})
...
async def update_area(area_id: str, area_data: AreaUpdateModel, current_user: dict = Depends(verify_token)):
    """تحديث منطقة موجودة"""
    try:
        # التحقق من وجود المنطقة
        existing_area = areas_collection.find_one({"id": area_id})
        ...
        if area_data.code is not None:
            # التحقق من عدم تكرار الكود
            if area_data.code != existing_area.get("code"):
                existing_code = areas_collection.find_one({"code": area_data.code.upper()})
                if existing_code:
                    raise HTTPException(status_code=400, detail="كود المنطقة موجود بالفعل")
        ...
        # الحصول على البيانات المحدثة
        updated_area = areas_collection.find_one({"id": area_id}, {"_id": 0})
        
        # إضافة معلومات الخط
        if updated_area.get("line_id"):
            line = lines_collection.find_one({"id": updated_area["line_id"]})
...
async def delete_line(line_id: str, current_user: dict = Depends(verify_token)):
    """حذف خط (إلغاء تفعيل)"""
    try:
        # التحقق من وجود الخط
        line = lines_collection.find_one({"id": line_id})
        ...
        areas_count = areas_collection.count_documents({"line_id": line_id, "is_active": True})
        if areas_count > 0:
...
async def delete_area(area_id: str, current_user: dict = Depends(verify_token)):
    """حذف منطقة (إلغاء تفعيل)"""
    try:
        # التحقق من وجود المنطقة
        area = areas_collection.find_one({"id": area_id})
        if not area:
            raise HTTPException(status_code=404, detail="المنطقة غير موجودة")
        
        # التحقق من وجود عيادات مرتبطة
        clinics_count = clinics_collection.count_documents({"area_id": area_id})
        if clinics_count > 0:
...
async def get_lines_areas_statistics(current_user: dict = Depends(verify_token)):
    """الحصول على إحصائيات الخطوط والمناطق"""
    try:
        # إحصائيات الخطوط
        total_lines = lines_collection.count_documents({"is_active": True})
        lines_with_manager = lines_collection.count_documents({"is_active": True, "manager_id": {"$ne": None}})
        
        # إحصائيات المناطق
        total_areas = areas_collection.count_documents({"is_active": True})
        areas_with_manager = areas_collection.count_documents({"is_active": True, "manager_id": {"$ne": None}})
        
        # إحصائيات العيادات حسب الخطوط
        lines_with_clinics = list(lines_collection.aggregate([
        ...
        lines_with_reps = list(lines_collection.aggregate([
            {"$match": {"is_active": True}},
            {"$lookup": {
                "from": "users",
                "localField": "id",
