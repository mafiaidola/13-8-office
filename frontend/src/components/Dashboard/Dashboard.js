...
const Dashboard = ({ user, language, isRTL, setActiveTab }) => {
  ...
  const loadEnhancedDashboardData = async () => {
    try {
      ...
      console.log(`🔄 Loading dashboard data for time filter: ${timeFilter}`);
      
      // Load dashboard stats from multiple endpoints (remove time_filter param for now)
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div class="footer">
          ...
          <p>🏥 النظام الطبي المتكامل - إدارة الأنشطة الطبية المتقدم</p>
        </div>
      </body>
      </html>
    `;
